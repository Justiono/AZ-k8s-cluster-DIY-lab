...
echo "Step 4 - Creating an edge node from the boot image"
echo "=================================================================="
# Define a credential object
$securePassword = ConvertTo-SecureString 'MyP@55w0rd' -AsPlainText -Force
$cred = New-Object System.Management.Automation.PSCredential ("azuser", $securePassword)

$VMNameTmp = $AzQualifier + "edge"
$pip = New-AzPublicIpAddress `
  -ResourceGroupName $AzRgName `
  -Location $AzLoc `
  -AllocationMethod Dynamic `
  -IdleTimeoutInMinutes 4 `
  -Name "$VMNameTmp$(Get-Random)"
...
For ($i=1; $i -le $NumOfMasterNode; $i++) {
    ...
    Invoke-AzVMRunCommand `
      ...
      -VMName $VMNameTmp `
      -CommandId "RunShellScript" `
      -ScriptPath "./K8SMasterConfig.bash"
    }
 
echo "Step 6 - Creating worker node(s) from the boot image"
echo "=================================================================="

For ($i=1; $i -le $NumOfWorkerNode; $i++) {
    # Create a virtual network card (without public IP address)
    $VMNameTmp = $AzQualifier + "worker" + $i
    $nic0 = New-AzNetworkInterface `
      -Name $VMNameTmp"Nic" `
      -ResourceGroupName $AzRgName `
      -Location $AzLoc `
    ...
    Invoke-AzVMRunCommand `
      ...
      -VMName $VMNameTmp `
      -CommandId "RunShellScript" `
      -ScriptPath "./K8SWorkerConfig.bash"
    }
